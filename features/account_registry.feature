Feature: Account registry
   ...
   Scenario: User is able to update name of already created account
        Given Account registry is empty
        And I create an account using name: "janey", last name: "voe", pesel: "44050372345"
        When I update "name" of account with pesel: "44050372345" to "jane"
        Then Account with pesel "44050372345" has "name" equal to "jane"

    Scenario: Created account has all fields correctly set
        Given Account registry is empty
        When I create an account using name: "jan", last name: "nowak", pesel: "90010112345"
        Then Account with pesel "90010112345" exists in registry
        And Account with pesel "90010112345" has "name" equal to "jan"
        And Account with pesel "90010112345" has "surname" equal to "nowak"
